...
class RoIHeads_Relation(torch.nn.Module):
    def __init__(self,
                 box_roi_pool,
                 box_head,
                 box_predictor,
        ...
        super(RoIHeads_Relation, self).__init__()

        self.box_similarity = box_op.box_iou
        self.FC_1 = TwoMLPHead(out_channels * resolution ** 2,
                               representation_size
                               )
        self.FC_2 = TwoMLPHead(representation_size,
                               representation_size
                               )
        self.relation_module_1 = Relation(f_A_shape=representation_size,
                                          roi_out_size=Config.roi_out_size)
        self.relation_module_2 = Relation(f_A_shape=representation_size,
                                          roi_out_size=Config.roi_out_size)
        # assign ground-truth boxes for each proposal
        self.proposal_matcher = Matcher(
            fg_iou_thresh,  # 0.5
    ...
    def forward(self,
        ...
        # 通过roi_pooling后的两层全连接层
        # box_features = self.box_head(box_features)
        box_features = self.FC_1(box_features)
        box_features = self.relation_module_1(box_features, proposals)
        box_features = self.FC_2(box_features)
        box_features = self.relation_module_2(box_features, proposals)
        # 接着分别预测目标类别和边界框回归参数
